...
<div class="w w1 header clearfix">
    <div id="logo"><a href="/"><img src="/images/moodshop-logo.png" alt="心情杂货铺"></a></div>
</div>
...
<form id="orderForm" class="hide" action="/order/create" method="post">
		...
		<c:forEach items="${cartList }" var="cart" varStatus="status">
			...
			<input type="hidden" name="orderItems[${status.index}].totalFee" value="${cart.price * cart.num+20}"/>
		...
		<input type="hidden" name="payment" value="<fmt:formatNumber groupingUsed="false" maxFractionDigits="2" minFractionDigits="2" value="${totalPrice/100+20}"/>"/>
		<input type="hidden" name="postFee" value="0"/>
		<input type="hidden" name="orderShipping.receiverName" value="史彦芊"/>
		<input type="hidden" name="orderShipping.receiverMobile" value="15111111111"/>
		<input type="hidden" name="orderShipping.receiverState" value="山西"/>
		<input type="hidden" name="orderShipping.receiverCity" value="太原"/>
		<input type="hidden" name="orderShipping.receiverDistrict" value="尖草坪区"/>
		<input type="hidden" name="orderShipping.receiverAddress" value="太原工业学院"/>
</form>
   			...
   			<ul class="ui-switchable-panel-main" id="consignee-list">
				...
				<li class="ui-switchable-panel" id="consignee_index_137617472"
					...
					<div class="consignee-item item-selected"
						...
						<div class="user-name">
							<div class="fl">
								<strong limit="4">史彦芊</strong>&nbsp;&nbsp;收
							</div>
							<div class="fr">151****8888</div>
							<div class="clr"></div>
						</div>
						<div class="mt10" limit="15">太原尖草坪区</div>
						<div class="adr-m" limit="30">太原工业学院</div>
...
<div class="shopping-list ABTest">
	<div class="goods-list">
     <!--配送方式-->
    <h4 class="vendor_name_h" id="0">商家：心情杂货铺</h4>		         
		...
		<div class="mode-tab-nav">
			<ul>
				<li class="mode-tab-item " id="jd_shipment_item"
					...
					id="jdShip-span-tip" class="m-txt">心情杂货铺快递<i
						class='qmark-icon qmark-tip'
						data-tips='由心情杂货铺负责配送，速度很快'></i></span><b></b></li>
			</ul>
		</div>
	</div>
</div>
			...
			<div class="statistic fr">
				...
				<div class="list">
					<span>应付总额：</span> <em class="price" id="sumPayPriceId">
						￥<fmt:formatNumber value="${(totalPrice / 100)+20}" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/></em>
				</div>
			</div>
			<div class="clr"></div>
		</div>
      ...
      <div class="sticky-wrap">
          ...
          <button type="submit" class="checkout-submit btn-1" 
          	...
          	提交订单
                    ...
                    <span class="total">应付总额：<strong id="payPriceId">￥<fmt:formatNumber value="${(totalPrice / 100)+20}" maxFractionDigits="2" minFractionDigits="2" groupingUsed="true"/></strong>
          </span>
