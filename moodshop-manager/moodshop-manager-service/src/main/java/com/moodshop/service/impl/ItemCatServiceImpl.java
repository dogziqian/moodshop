...
public class ItemCatServiceImpl implements ItemCatService {
	...
	public MSResult insertCatgory(Long parentId, String name) {
		...
		logger.info("查询到的父节点为："+parentNode);
	...
	public MSResult deleteCategory(Long id) {
		...
		if(itemCat.getIsParent()){
			...
			if(list.size()<=1){
				...
				TbItemCat parentItemCat=new TbItemCat();
				parentItemCat.setId(itemCat.getParentId());
				parentItemCat.setIsParent(false);
				logger.info("更新父节点状态.......");
				itemCatMapper.updateByPrimaryKeySelective(parentItemCat);
			}
			logger.info("删除该节点.......");
